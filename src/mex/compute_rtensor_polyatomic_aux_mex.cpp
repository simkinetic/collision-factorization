...
struct Config {
    ...
    // Auxiliary Laplace s-node (aux MEX only). The extended factor E^{-zhat} is resolved by
    //   E^{-zhat} = 1/Gamma(zhat) int_0^inf s^{zhat-1} e^{-sE} ds .
    // Per s-node the kernel carries the velocity part e^{-s*(1/2)|u|^2}, while the internal/post
...
inline void compute_polyatomic_inner(const Config& cfg, ThreadScratch& tb, 
    ...
    for (int I_idx = 0; I_idx < cfg.N_I_nodes; ++I_idx) {
        ...
        for (int J_idx = 0; J_idx < cfg.N_J_nodes; ++J_idx) {
            ...
            // Auxiliary Laplace velocity factor e^{-s*(1/2)|u|^2}. s_laplace==0 -> 1 (base build).
            // Scales gain and loss equally, so conservation structure is preserved.
            if (cfg.s_laplace != 0.0) factor *= std::exp(-cfg.s_laplace * 0.5 * u_mag * u_mag);
