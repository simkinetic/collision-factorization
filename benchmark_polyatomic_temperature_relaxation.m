...
write_relaxation_table(fullfile(outdir,'table_temperature_relaxation.tex'), ...
    sweepA, sweepB, delta_B, Teq_str, lamLT_str, zeta_A);

fprintf('Saved: %s, per-case CSVs in %s, and results/table_temperature_relaxation.tex\n', ...
...
function write_relaxation_table(fname, sweepA, sweepB, delta_B, Teq_str, lamLT_str, zeta_A)
    ...
    % rate vs omega) for the manuscript. Emitted as results/table_temperature_relaxation.tex
    % in the same booktabs style as the other hydrodynamic-quantity tables.
    fid = fopen(fname, 'w');
